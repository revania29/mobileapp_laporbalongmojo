...
class _DetailLaporanScreenState extends State<DetailLaporanScreen> {
  ...
  Color _getStatusColor(String status) {
    switch (status.toLowerCase()) {
      case 'belum terdaftar': return Colors.orangeAccent;
      case 'diproses': return Colors.blueAccent;
      case 'selesai': return Colors.greenAccent;
      case 'tolak': return Colors.redAccent;
  ...
  Future<void> _updateStatus(String newStatus) async {
    ...
    try {
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Status: ${newStatus.toUpperCase()}"), backgroundColor: Colors.green));
    } catch (e) {
  ...
  Future<void> _deleteLaporan() async {
    ...
    if (confirm == true) {
      try {
        ...
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Laporan dihapus"), backgroundColor: Colors.red));
      } catch (e) {
            ...
            children: [
              ...
              GlassCard(
                ...
                child: Column(
                  ...
                  children: [
                    if (hasImage)
                      Container(height: 250, width: double.infinity, margin: const EdgeInsets.only(bottom: 20), decoration: BoxDecoration(borderRadius: BorderRadius.circular(15), color: Colors.black26, image: DecorationImage(image: NetworkImage('${ApiService.publicBaseUrl}${widget.laporan.fotoUrl}'), fit: BoxFit.cover))),
                    
                    Text(widget.laporan.judul, style: const TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.bold)),
                    const SizedBox(height: 10),
                    Row(children: [Icon(Icons.person, color: statusColor, size: 16), const SizedBox(width: 5), Text("Pelapor: ${widget.laporan.pelapor}", style: const TextStyle(color: Colors.white70, fontSize: 13))]),
                    const SizedBox(height: 5),
                    Text(DateFormat('EEEE, dd MMM yyyy').format(widget.laporan.tanggal), style: const TextStyle(color: Colors.white54, fontSize: 12)),
                    const Divider(color: Colors.white24, height: 30),
                    Text(widget.laporan.deskripsi, style: const TextStyle(color: Colors.white, fontSize: 16), textAlign: TextAlign.justify),
                  ],
                ),
              ),
              ...
              // 2. DROPDOWN STATUS (BAWAH - UNGU)
              const Align(alignment: Alignment.centerLeft, child: Text("Update Status:", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold))),
              const SizedBox(height: 10),
              ...
              Container(
                ...
                child: DropdownButtonHideUnderline(
                  child: DropdownButton<String>(
                    ...
                    isExpanded: true,
                    items: ['belum terdaftar', 'diproses', 'selesai', 'tolak'].map((String value) => DropdownMenuItem<String>(value: value, child: Text(value.toUpperCase(), style: const TextStyle(color: Colors.white)))).toList(),
              ...
              // 3. TOMBOL HAPUS (HANYA JIKA SELESAI/TOLAK)
              if (canDelete)
                SizedBox(
                  width: double.infinity,
                  child: ElevatedButton.icon(
                    style: ElevatedButton.styleFrom(backgroundColor: Colors.redAccent.withOpacity(0.9), padding: const EdgeInsets.symmetric(vertical: 15), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(15))),
